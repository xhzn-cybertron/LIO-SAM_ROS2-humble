...
public:
    ...
    {
        // RCLCPP_INFO(get_logger(), "Receive a imuMsg...");
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odometryMsg)
    {
        ...
        // RCLCPP_INFO(get_logger(), "Receive a odometryMsg...");
        odomQueue.push_back(*odometryMsg);
    }

    void cloudHandler(const sensor_msgs::msg::PointCloud2::SharedPtr laserCloudMsg)
    {
        // RCLCPP_INFO(get_logger(), "Receive a laserCloudMsg...");
        if (!cachePointCloud(laserCloudMsg))
            return;

        // RCLCPP_INFO(get_logger(), "bf deskewInfo");
        if (!deskewInfo())
            return;

        // RCLCPP_INFO(get_logger(), "bf projectPointCloud");
        projectPointCloud();

        // RCLCPP_INFO(get_logger(), "bf cloudExtraction");       
        cloudExtraction();

        // RCLCPP_INFO(get_logger(), "bf publishClouds");
        publishClouds();

        // RCLCPP_INFO(get_logger(), "bf resetParameters");
        resetParameters();

        // RCLCPP_INFO(get_logger(), "Exit a laserCloudMsg...");
    ...
    void publishClouds()
    {
        ...
        // RCLCPP_INFO(get_logger(), "Publishing a CloudInfo...");
    }
};
